...
namespace Argo::Utilities {
    ...
    CommonColor ColorConverter::FromRGBA(const Vec4 rgba) { return ColorConverter::FromRGBA(&rgba); }

    CommonColor ColorConverter::FromRGBA(const Vec4* rgba) {
        const CommonColor color = {rgba->w / maxValue, rgba->x / maxValue, rgba->y / maxValue, rgba->z};
        return color;
    }

    CommonColor ColorConverter::FromHex(const unsigned int hex) { return ColorConverter::FromHex(&hex); }

    CommonColor ColorConverter::FromHex(const unsigned int* hex) {
    ...
    CommonColor ColorConverter::FromHex(const std::string&hex) { return FromHex(&hex); }

    CommonColor ColorConverter::FromHex(const std::string* hex) {
