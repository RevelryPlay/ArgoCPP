...
//            g_logger_assert((pixels != nullptr), "STB failed to load image: %s\n-> STB Failure Reason: %s", res.filepath.c_str(), stbi_failure_reason());
            ...
            {
                fprintf(stderr,"Unknown number of channels '%d' in image '%s'.", res.numChannels, res.filepath.c_str());
        ...
        void init()
        {
            if (!texturedCubeShader.compileAndLink("resources/shaders/vertex/cube.glsl", "resources/shaders/fragment/cube.glsl"))
            {
                texturedCubeShader.destroy();
                fprintf(stderr,"Failed to compile the shader program.");
            }

            createDefaultCube();

            for (auto& filepath : std::filesystem::directory_iterator("resources/images"))
            {
